...
const Navbar = () => {
  ...
  const userMenuLinks = user ? [
    ...
    { to: "/add-crop", label: "Add Crops", icon: LuTrees },
    { to: "/my-posts", label: "My Posts", icon: LuLayoutDashboard },
    { to: "/my-interests", label: "My Interests", icon: LuSettings },
  ] : [];

  return (
    <header className="bg-gradient-to-b from-green-50 to-white shadow-sm sticky top-0 z-50 backdrop-blur-sm bg-opacity-95">
      <MyContainer>
          ...
          <div className="px-4 py-3 space-y-1">
            {primaryLinks.map((link) => (
              <MyLinks key={link.to} to={link.to} onClick={() => setMobileOpen(false)}
                className="flex items-center gap-3 px-4 py-2 text-gray-700 hover:bg-green-50 hover:text-green-600 rounded-lg transition flex items-center gap-2">
            ...
            {moreLinks.map((link) => (
              <MyLinks
                ...
                className="block px-4 py-2 text-gray-700 hover:bg-green-50 hover:text-green-600 rounded-lg transition flex items-center gap-2">
                <link.icon className="w-4 h-4" />
                <span>
                  {link.label}
                </span>
